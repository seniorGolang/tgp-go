...
await client.batch([
    {
        rpcRequest: userService.getUserRequest('user-id-1'),
        retHandler: (error, result) => {
            if (error) {
    ...
    {
        rpcRequest: userService.getUserRequest('user-id-2'),
    },
]);
```
...
await client.batch([
    {
        rpcRequest: userService.getUserRequest('1'),
        retHandler: (error, result) => {
            if (error) {
                console.error('Error:', error);
                return;
    ...
    {
        rpcRequest: userService.getUserRequest('2'),
        retHandler: (error, result) => {
            if (error) {
                console.error('Error:', error);
                return;
