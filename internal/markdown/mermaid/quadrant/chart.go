...
func (ch *Chart) String() (s string) {
	return strings.Join(ch.body, internal.LineFeed())
}

func (ch *Chart) Error() (err error) {
	return ch.err
}

func (ch *Chart) Build() (err error) {

	if _, writeErr := fmt.Fprint(ch.dest, ch.String()); writeErr != nil {
		if ch.err != nil {
			return fmt.Errorf("failed to write: %w: %s", writeErr, ch.err.Error()) //nolint:wrapcheck
		}
