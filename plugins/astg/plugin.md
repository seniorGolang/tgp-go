...
| `tagNoOmitempty`          | Отключение тега `omitempty` для полей ответа              | `// @tg tagNoOmitempty`               |
...
Для упрощения отладки и анализа структуры проекта, при включенном уровне логирования `debug`, плагин автоматически сохраняет полную структуру распарсенного контракта в файл `.tg/project.json` в корне проекта. Этот файл содержит полную модель проекта в формате JSON, включая все контракты, методы, типы, аннотации и их связи.

Файл создается автоматически при каждом запуске плагина с уровнем логирования `debug` и может быть использован для:

- Анализа структуры проекта
- Отладки проблем парсинга
- Проверки корректности извлеченных данных
- Интеграции с внешними инструментами анализа

**Примечание**: Файл `.tg/project.json` создается только при активном уровне `debug` и не влияет на производительность в production-режиме.
