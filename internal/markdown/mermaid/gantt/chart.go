...
func NewChart(w io.Writer, opts ...Option) (ch *Chart) {
...
func (c *Chart) String() (s string) {
	return strings.Join(c.body, internal.LineFeed())
}

func (c *Chart) Error() (err error) {
	return c.err
}

func (c *Chart) Build() (err error) {

	if _, writeErr := fmt.Fprint(c.dest, c.String()); writeErr != nil {
		if c.err != nil {
			return fmt.Errorf("failed to write: %w: %s", writeErr, c.err.Error()) //nolint:wrapcheck
		}
