...
func (r *ClientRenderer) RenderExchangeTypes(contract *model.Contract) (err error) {
	...
	for typeID, typ := range r.project.Types {
		if typ.ImportPkgPath == "" {
			continue
		}
		if typ.ImportPkgPath != contract.PkgPath && !strings.HasPrefix(typ.ImportPkgPath, contract.PkgPath+"/") {
			continue
		}
		name := typ.TypeName
		if name == "" {
			if idx := strings.LastIndex(typeID, ":"); idx >= 0 {
				name = typeID[idx+1:]
			} else {
				name = typeID
			}
		}
		_ = r.walkVariable(name, typ.ImportPkgPath, &model.Variable{TypeRef: model.TypeRef{TypeID: typeID}, Name: name}, nil, false)
	}
