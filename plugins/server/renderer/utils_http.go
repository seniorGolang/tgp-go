...
func (r *contractRenderer) methodHTTPMethod(method *model.Method) string {
	...
	httpMethod := model.GetAnnotationValue(r.project, r.contract, method, nil, TagMethodHTTP, "")
	if httpMethod == "" {
		return "post"
	}
...
func (r *contractRenderer) methodHTTPPath(method *model.Method) string {
	...
	prefix := model.GetAnnotationValue(r.project, r.contract, nil, nil, TagHttpPrefix, "")
	methodPath := model.GetAnnotationValue(r.project, r.contract, method, nil, TagHttpPath, "/"+toLowerCamel(r.contract.Name)+"/"+toLowerCamel(method.Name))
	if prefix != "" {
...
func (r *contractRenderer) methodJsonRPCPath(method *model.Method) string {
	...
	prefix := model.GetAnnotationValue(r.project, r.contract, nil, nil, TagHttpPrefix, "")
	urlPath := model.GetAnnotationValue(r.project, r.contract, method, nil, TagHttpPath, "/"+toLowerCamel(r.contract.Name)+"/"+toLowerCamel(method.Name))
