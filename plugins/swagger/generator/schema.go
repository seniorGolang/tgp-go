...
func (g *generator) variableToSchema(variable *model.Variable, pkgPath string, isArgument bool) (schema *types.Schema) {
	...
	if variable.IsSlice || variable.ArrayLen > 0 {
		itemVar := &model.Variable{
			...
			NumberOfPointers: variable.NumberOfPointers,
		}
		itemSchema := g.variableToSchema(itemVar, pkgPath, isArgument)
		if itemSchema == nil {
			return nil
		}
		return &types.Schema{
	...
	if variable.MapKeyID != "" && variable.MapValueID != "" {
		valueVar := &model.Variable{
			Name:             variable.Name,
			TypeID:           variable.MapValueID,
			NumberOfPointers: variable.NumberOfPointers,
		}
		valueSchema := g.variableToSchema(valueVar, pkgPath, isArgument)
		if valueSchema == nil {
			return nil
		}
		return &types.Schema{
	...
	typeInfo, found := g.project.Types[variable.TypeID]

	if !found {
		if types.IsExcludedTypeID(variable.TypeID, g.project) {
			if schema = g.basicTypeToSchema(variable.TypeID, variable.Annotations); schema == nil {
				return
			}
	...
	if hasCustomMarshaler && !isExcluded {
		typeName := g.normalizeTypeName(typeInfo, variable.TypeID, pkgPath)
	...
	if typeInfo.ArrayOfID != "" && types.IsExcludedTypeID(typeInfo.ArrayOfID, g.project) {
		if schema = g.basicTypeToSchema(typeInfo.ArrayOfID, variable.Annotations); schema == nil {
			return
		}
		if variable.NumberOfPointers > 0 {
			return &types.Schema{
	...
	case model.TypeKindAlias:
		if typeInfo.AliasOf != "" {
			baseTypeVar := &model.Variable{TypeID: typeInfo.AliasOf}
			baseSchema := g.variableToSchema(baseTypeVar, pkgPath, isArgument)
			if baseSchema == nil {
				return
			}
		...
		return g.basicTypeToSchema(variable.TypeID, variable.Annotations)

	case model.TypeKindStruct:
		if schema = g.structTypeToSchema(typeInfo, variable.Annotations); schema == nil {
			return
		}
		if variable.NumberOfPointers > 0 {
			return &types.Schema{
	...
	case model.TypeKindMap:
		if typeInfo.MapValueID != "" {
			valueVar := &model.Variable{
				Name:   variable.Name,
				TypeID: typeInfo.MapValueID,
			}
			valueSchema := g.variableToSchema(valueVar, pkgPath, isArgument)
			if valueSchema == nil {
				return
