...
// RenderJsonRPCLibrary генерирует JSON-RPC библиотеку для TypeScript клиента
func (r *ClientRenderer) RenderJsonRPCLibrary() error {
	...
	// Генерируем jsonrpc/client.ts
	if err := r.renderJsonRPCClient(jsonrpcDir); err != nil {
		return err
	}

	// Генерируем jsonrpc/utils/jsonrpc.ts
	if err := r.renderJsonRPCUtils(utilsDir); err != nil {
		return err
	}

	// Генерируем jsonrpc/utils/ts.ts
...
// renderJsonRPCClient генерирует jsonrpc/client.ts
func (r *ClientRenderer) renderJsonRPCClient(jsonrpcDir string) error {
	...
	// Генерируем класс JsonRpcClient без generic параметра (T не используется)
	classStmt := tsg.NewStatement().
		...
		callMethod := tsg.NewStatement().
			...
			AsyncMethodWithParams("exec", callMethodParams, callReturnType, func(bg *tsg.Group) {
				...
				// Получаем заголовки (поддерживает статичные и динамические)
		...
		batchMethod := tsg.NewStatement().
			...
			AsyncMethodWithParams("callBatch", batchParams, batchReturnType, func(bg *tsg.Group) {
				...
				// Создаём Map из массива ответов
...
// renderJsonRPCUtils генерирует jsonrpc/utils/jsonrpc.ts
...
// renderJsonRPCTsUtils генерирует jsonrpc/utils/ts.ts
